...
def render_narrative(
    ...
    # {{?top_features}} placeholder. The Part B template expects this
    ...
    scenario, name, version = template_ref.split("/")
    body = {
    ...
    if response.status_code >= 400:
        logger.error("[ORCH] HTTP %s response body: %s", response.status_code, response.text)
