...
## Exercise 3: Knowledge Graph (Conceptual)

> **Note:** Knowledge Graph requires HANA Cloud paid tier (3+ vCPU, 45GB). Covered conceptually.
...
```
Chase → has_complaint → COMP-10001
COMP-10001 → about_product → Credit Card
Credit Card → is_type_of → Financial Product
```

### SPARQL Query Example

```sparql
SELECT ?company ?complaint ?product
WHERE {
    ?company <has_complaint> ?complaint .
    ?complaint <about_product> ?product .
}
```
...
|----------|--------------|----------|
| VectorRAG | Find similar text | "Documents like this" |
| GraphRAG | Traverse relationships | "Facts about this entity" |
| HybridRAG | Both combined | Enterprise AI (semantic + factual) |

### Key Insight

> **VectorRAG finds similar content. GraphRAG finds true facts.**
> HANA Cloud combines both in one query: SQL + SPARQL + Vector functions.

---

## Next Steps

1. **AI Core Embeddings:** Deploy text-embedding-ada-002 for production vectors
2. **Joule Integration:** Natural language queries on vector data
3. **Knowledge Graph Hands-on:** When paid tier is available
