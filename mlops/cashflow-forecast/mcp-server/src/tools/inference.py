...
Thin wrapper that uses gen-ai-hub-sdk's proxy client for OAuth2/token
management, then issues a plain `requests.post` against any AI Core
...
def call_explain(company_code: str) -> dict:
    ...
    Returns:
        Full JSON payload as returned by /v2/explain.
    ...
    # SDK 3.1.1: pull a fresh authenticated header set from the proxy.
    # request_header includes Authorization (Bearer token, refreshed per
    # call) plus AI-Resource-Group from the proxy's default. We override
    # the resource group here for cross-RG calls.
